...
- (NSString *)ht_interceptionByGBK:(int)length {
    ...
    for (int i = 0; self.length > i; i++) {
        ...
        if ([currentString ht_stringCount] <= length) {
...
char * GetPYChar(const unsigned char *array)
{
...
- (NSString *)ht_getChineseCaptialChar
{
    ...
    if (str[0] < 0 && !isalpha(str[0])) {
        return [NSString stringWithCString:GetPYChar((const unsigned char *)str) encoding:enc];
    } else {
...
- (NSString *)ht_urlEncode {
    return [self ht_urlEncodeUsingEncoding:NSUTF8StringEncoding];
}
...
- (NSString *)ht_urlDecode {
    return [self ht_urlDecodeUsingEncoding:NSUTF8StringEncoding];
}
...
- (NSString *)ht_clipFitStringForLabel:(CGSize)labelSize font:(UIFont *)font {
    ...
    CGFloat originalWidth = [self ht_singleLineWidthWithFont:font];
    if (originalWidth <= maxWidth) return [self copy];
    CGFloat pointStrLength = [@"" ht_singleLineWidthWithFont:font];
    ...
    for (NSInteger i = originalString.length - 1;i >= 0;i--){
        ...
        CGFloat tempStringWidth = [tempString ht_singleLineWidthWithFont:font];
