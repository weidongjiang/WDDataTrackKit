...
#define HTMainThreadAssert() NSAssert([NSThread isMainThread], @"HTProgressHUD needs to be accessed on the main thread.");

CGFloat const HTProgressMaxOffset = 1000000.f;

static const CGFloat HTDefaultPadding = 4.f;
static const CGFloat HTDefaultLabelFontSize = 16.f;
static const CGFloat HTDefaultDetailsLabelFontSize = 12.f;
...
@interface HTProgressHUDRoundedButton : UIButton
@end
...
+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    HTProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    HTProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (HTProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            return (HTProgressHUD *)subview;
        }
    }
    return nil;
}
...
- (void)commonInit {
    ...
    _animationType = HTProgressHUDAnimationFade;
    _mode = HTProgressHUDModeIndeterminate;
    _margin = 20.0f;
    _opacity = 1.f;
...
- (void)animateIn:(BOOL)animatingIn withType:(HTProgressHUDAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == HTProgressHUDAnimationZoom) {
        type = animatingIn ? HTProgressHUDAnimationZoomIn : HTProgressHUDAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == HTProgressHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == HTProgressHUDAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == HTProgressHUDAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == HTProgressHUDAnimationZoomOut) {
            bezelView.transform = small;
        }
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
...
- (void)done {
    ...
    HTProgressHUDCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<HTProgressHUDDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    HTBackgroundView *backgroundView = [[HTBackgroundView alloc] initWithFrame:self.bounds];
    backgroundView.style = HTProgressHUDBackgroundStyleSolidColor;
    ...
    HTBackgroundView *bezelView = [HTBackgroundView new];
    ...
    label.font = [UIFont boldSystemFontOfSize:HTDefaultLabelFontSize];
    ...
    detailsLabel.font = [UIFont boldSystemFontOfSize:HTDefaultDetailsLabelFontSize];
    detailsLabel.opaque = NO;
    detailsLabel.backgroundColor = [UIColor clearColor];
    _detailsLabel = detailsLabel;

    UIButton *button = [HTProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
    button.titleLabel.textAlignment = NSTextAlignmentCenter;
    button.titleLabel.font = [UIFont boldSystemFontOfSize:HTDefaultDetailsLabelFontSize];
...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [indicator isKindOfClass:[HTRoundProgressView class]];

    HTProgressHUDMode mode = self.mode;
    if (mode == HTProgressHUDModeIndeterminate) {
    ...
    else if (mode == HTProgressHUDModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[HTBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
    else if (mode == HTProgressHUDModeDeterminate || mode == HTProgressHUDModeAnnularDeterminate) {
        if (!isRoundIndicator) {
            // Update to determinante indicator
            [indicator removeFromSuperview];
            indicator = [[HTRoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
        if (mode == HTProgressHUDModeAnnularDeterminate) {
            [(HTRoundProgressView *)indicator setAnnular:YES];
        }
    }
    else if (mode == HTProgressHUDModeCustomView && self.customView != indicator) {
        // Update custom view indicator
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self.bezelView addSubview:indicator];
    }
    else if (mode == HTProgressHUDModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
...
#pragma clang diagnostic pop
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[HTProgressHUD class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[HTProgressHUD class]]];
#endif

        if (appearance.color == nil) {
            ((UIActivityIndicatorView *)indicator).color = color;
        }
    } else if ([indicator isKindOfClass:[HTRoundProgressView class]]) {
        HTRoundProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [HTRoundProgressView appearanceWhenContainedIn:[HTProgressHUD class], nil];
#else
        appearance = [HTRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[HTProgressHUD class]]];
#endif
        if (appearance.progressTintColor == nil) {
            ((HTRoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((HTRoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[HTBarProgressView class]]) {
        HTBarProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [HTBarProgressView appearanceWhenContainedIn:[HTProgressHUD class], nil];
#else
        appearance = [HTBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[HTProgressHUD class]]];
#endif
        if (appearance.progressColor == nil) {
            ((HTBarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((HTBarProgressView *)indicator).lineColor = color;
        }
    } else {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
    HTBackgroundView *bezelView = self.bezelView;
...
- (void)updatePaddingConstraints {
    ...
    [self.paddingConstraints enumerateObjectsUsingBlock:^(NSLayoutConstraint *padding, NSUInteger idx, BOOL *stop) {
        ...
        padding.constant = (firstVisible && (secondVisible || hasVisibleAncestors)) ? HTDefaultPadding : 0.f;
        hasVisibleAncestors |= secondVisible;
        
        if (self.labelPadding > 0 && firstView == self.label) {
            padding.constant = self.labelPadding;
        }
        if (self.buttonPadding > 0 && firstView == self.button) {
...
- (void)setMode:(HTProgressHUDMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == HTProgressHUDModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
@implementation HTRoundProgressView
...
- (UILabel *)progressLabel {
    if (!_progressLabel) {
        _progressLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 30, 30)];
...
@implementation HTBarProgressView
...
@implementation HTBackgroundView
...
- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_7_0) {
            _style = HTProgressHUDBackgroundStyleBlur;
            ...
            _style = HTProgressHUDBackgroundStyleSolidColor;
...
- (void)setStyle:(HTProgressHUDBackgroundStyle)style {
    if (style == HTProgressHUDBackgroundStyleBlur && kCFCoreFoundationVersionNumber < kCFCoreFoundationVersionNumber_iOS_7_0) {
        style = HTProgressHUDBackgroundStyleSolidColor;
    }
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
...
- (void)updateForBackgroundStyle {
    HTProgressHUDBackgroundStyle style = self.style;
    if (style == HTProgressHUDBackgroundStyleBlur) {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == HTProgressHUDBackgroundStyleBlur) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
            self.backgroundColor = self.color;
        } else {
#if !TARGET_OS_TV
...
@implementation HTProgressHUD (Deprecated)

#pragma mark - Class

+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated {
    NSArray *huds = [HTProgressHUD allHUDsForView:view];
    for (HTProgressHUD *hud in huds) {
        hud.removeFromSuperViewOnHide = YES;
        [hud hideAnimated:animated];
    }
    return [huds count];
...
- (void)showAnimated:(BOOL)animated whileExecutingBlock:(dispatch_block_t)block onQueue:(dispatch_queue_t)queue completionBlock:(nullable HTProgressHUDCompletionBlock)completion {
...
- (BOOL)dimBackground {
    HTBackgroundView *backgroundView = self.backgroundView;
    UIColor *dimmedColor =  [UIColor colorWithWhite:0.f alpha:.2f];
    return backgroundView.style == HTProgressHUDBackgroundStyleSolidColor && [backgroundView.color isEqual:dimmedColor];
}

- (void)setDimBackground:(BOOL)dimBackground {
    HTMainThreadAssert();
    self.backgroundView.style = HTProgressHUDBackgroundStyleSolidColor;
...
@implementation HTProgressHUDRoundedButton
